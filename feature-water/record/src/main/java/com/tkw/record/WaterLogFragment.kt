    ...
    fun initView() {
        with(dataBinding) {
            ...
            TabLayoutMediator(tabLayout, viewPager) { tab, position ->
                when(position) {
                    0 -> tab.text = getString(com.tkw.base.R.string.log_day_title)
                    1 -> tab.text = getString(com.tkw.base.R.string.log_week_title)
                    else -> tab.text = getString(com.tkw.base.R.string.log_month_title)
                }
            }.attach()
        }
    }
